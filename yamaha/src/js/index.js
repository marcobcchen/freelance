$(function(){
  ...
  function start(){
    ...
    var isIntro = localStorage.getItem('intro');
  ...
  function clickPopup(ele, e){
    ...
    if(popup){
      ...
      if(popup == "good_home"){
        ...
        $('.popup-basic .pic-'+popup).css('display', 'block');
  ...
  function changeBikeColor(){
    ...
    let url = 'showroom/';
    // if(lsBikeType == 0){
    //   url = 'showroom/jog.html';
    //   url = 'showroom/jog.html';
    // }else{
    //   url = 'showroom/axisz.html';
    //   url = 'showroom/axisz.html';
    // }
    $('.flag-2').attr('href', url);
    $('.navigate a:first-child').attr('href', url);
  ...
  function initPopup(){
    ...
    TweenMax.set($('.popup .con-1'), {autoAlpha: 1});
    TweenMax.set($('.popup .con-2'), {autoAlpha: 0});

    var popupTl = new TimelineMax({repeat: -1, repeatDelay: 3});

    popupTl.addLabel('flag1');
    popupTl.to($('.popup .con-1'), 0.5, {autoAlpha: 0}, 'flag1');
    popupTl.to($('.popup .con-2'), 0.5, {autoAlpha: 1}, 'flag1');
    popupTl.addLabel('flag2');
    popupTl.to($('.popup .con-1'), 0.5, {autoAlpha: 1, delay: 6}, 'flag2');
    popupTl.to($('.popup .con-2'), 0.5, {autoAlpha: 0, delay: 6}, 'flag2');
  }

  function closePopup(){
    TweenMax.to($('.popup-basic'), 0.5, {scale: 1, autoAlpha: 0, ease: Back.easeOut, onComplete: function(){
      $('.popup').removeClass('open');
    }});
  }
  ...
  function initCarAnimate(){
    ...
    tl_car2.to($('.car2-l'), 4 * addSpeed, {left: '54.6%', top: '69%', ease: Linear.easeNone});
    tl_car2.to($('.car2-l'), 0, {alpha: 0});
    tl_car2.to($('.car2-r'), 0, {alpha: 1});
    tl_car2.to($('.car2-r'), 5 * addSpeed, {left: '67.4%', top: '53.2%', ease: Linear.easeNone});
    tl_car2.to($('.car2-r'), 0, {alpha: 0});
    tl_car2.to($('.car2-t'), 0, {alpha: 1});
    tl_car2.to($('.car2-t'), 4 * addSpeed, {left: '80%', top: '68.6%', ease: Linear.easeNone});
  }
